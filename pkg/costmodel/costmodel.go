...
const (
	// NOTE: For simplicity of resolving merge conflicts, I did not accept the changes from https://github.com/opencost/opencost/pull/2312 for this agent.
	queryRAMRequestsStr = `avg(
		label_replace(
			label_replace(
				...
				) by (%s,container,pod,%s,%s) , "container_name","$1","container","(.+)"
			), "pod_name","$1","pod","(.+)"
		)
	) by (%s,container_name,pod_name,%s,%s)`
	...
	* on (pod_name, namespace, %s) group_left(container) label_replace(avg(avg_over_time(kube_pod_status_phase{phase="Running", %s}[%s] %s)) by (pod,namespace,%s), "pod_name","$1","pod","(.+)")`
	queryPVRequestsStr = `avg(avg(kube_persistentvolumeclaim_info{volumename != "", %s}) by (persistentvolumeclaim, storageclass, %s, volumename, %s, kubernetes_node)
	*
	on (persistentvolumeclaim, %s, %s, kubernetes_node) group_right(storageclass, volumename)
	sum(kube_persistentvolumeclaim_resource_requests_storage_bytes{%s}) by (persistentvolumeclaim, %s, %s, kubernetes_node, kubernetes_name)) by (persistentvolumeclaim, storageclass, %s, %s, volumename, kubernetes_node)`
	...
	queryRAMAllocationByteHours = `
		label_replace(label_replace(
			...
			) by (%s,container,pod,%s,%s) * %f / 60 / 60
	...
	queryPVCAllocationFmt     = `sum(sum_over_time(pod_pvc_allocation{%s}[%s])) by (%s, %s, pod, persistentvolume, persistentvolumeclaim) * %f/60/60`
	...
	queryPodDaemonsets        = `sum(kube_pod_owner{owner_kind="DaemonSet", %s}) by (%s,pod,owner_name,%s)`
	queryPodJobs              = `sum(kube_pod_owner{owner_kind="Job", %s}) by (%s,pod,owner_name,%s)`
	queryServiceLabels        = `avg_over_time(service_selector_labels{%s}[%s])`
	queryZoneNetworkUsage     = `sum(increase(kubecost_pod_network_egress_bytes_total{internet="false", sameZone="false", sameRegion="true", %s}[%s] %s)) by (%s,pod_name,%s) / 1024 / 1024 / 1024`
	queryRegionNetworkUsage   = `sum(increase(kubecost_pod_network_egress_bytes_total{internet="false", sameZone="false", sameRegion="false", %s}[%s] %s)) by (%s,pod_name,%s) / 1024 / 1024 / 1024`
	queryInternetNetworkUsage = `sum(increase(kubecost_pod_network_egress_bytes_total{internet="true", %s}[%s] %s)) by (%s,pod_name,%s) / 1024 / 1024 / 1024`
...
func (cm *CostModel) ComputeCostData(cli prometheusClient.Client, cp costAnalyzerCloud.Provider, window string, offset string, filterNamespace string) (map[string]*CostData, error) {
	...
	queryNetZoneRequests := fmt.Sprintf(queryZoneNetworkUsage, env.GetPromClusterFilter(), window, "", env.GetPromNamespaceLabel(), env.GetPromClusterLabel())
	queryNetRegionRequests := fmt.Sprintf(queryRegionNetworkUsage, env.GetPromClusterFilter(), window, "", env.GetPromNamespaceLabel(), env.GetPromClusterLabel())
	queryNetInternetRequests := fmt.Sprintf(queryInternetNetworkUsage, env.GetPromClusterFilter(), window, "", env.GetPromNamespaceLabel(), env.GetPromClusterLabel())
...
func (cm *CostModel) costDataRange(cli prometheusClient.Client, cp costAnalyzerCloud.Provider, window opencost.Window, resolution time.Duration, filterNamespace string, filterCluster string, remoteEnabled bool) (map[string]*CostData, error) {
	...
	queryRAMAlloc := fmt.Sprintf(queryRAMAllocationByteHours, env.GetPromNodeLabel(), env.GetPromClusterFilter(), resStr, env.GetDefaultPromNamespaceLabel(), env.GetPromNodeLabel(), env.GetPromClusterLabel(), scrapeIntervalSeconds) // DEFAULT NAMESPACE LABEL FOR VZ
	queryCPUAlloc := fmt.Sprintf(queryCPUAllocationVCPUHours, env.GetPromNodeLabel(), env.GetPromClusterFilter(), resStr, env.GetDefaultPromNamespaceLabel(), env.GetPromNodeLabel(), env.GetPromClusterLabel(), scrapeIntervalSeconds) // DEFAULT NAMESPACE LABEL FOR VZ
	...
	queryNetZoneRequests := fmt.Sprintf(queryZoneNetworkUsage, env.GetPromClusterFilter(), resStr, "", env.GetDefaultPromNamespaceLabel(), env.GetPromClusterLabel())         // DEFAULT NAMESPACE LABEL FOR VZ
	queryNetRegionRequests := fmt.Sprintf(queryRegionNetworkUsage, env.GetPromClusterFilter(), resStr, "", env.GetDefaultPromNamespaceLabel(), env.GetPromClusterLabel())     // DEFAULT NAMESPACE LABEL FOR VZ
	queryNetInternetRequests := fmt.Sprintf(queryInternetNetworkUsage, env.GetPromClusterFilter(), resStr, "", env.GetDefaultPromNamespaceLabel(), env.GetPromClusterLabel()) // DEFAULT NAMESPACE LABEL FOR VZ
	...
	resChJobs := ctx.QueryRange(fmt.Sprintf(queryPodJobs, env.GetPromClusterFilter(), env.GetPromNamespaceLabel(), env.GetPromClusterLabel()), start, end, resolution)
	resChDaemonsets := ctx.QueryRange(fmt.Sprintf(queryPodDaemonsets, env.GetPromClusterFilter(), env.GetPromNamespaceLabel(), env.GetPromClusterLabel()), start, end, resolution)
