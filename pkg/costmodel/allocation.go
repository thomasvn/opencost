...
const (
	...
	queryFmtGPUsUsageAvg                = `avg(avg_over_time(DCGM_FI_PROF_GR_ENGINE_ACTIVE{container!=""}[%s])) by (container, pod, %s, %s)`
	queryFmtGPUsUsageMax                = `max(max_over_time(DCGM_FI_PROF_GR_ENGINE_ACTIVE{container!=""}[%s])) by (container, pod, %s, %s)`
...
func (cm *CostModel) computeAllocation(start, end time.Time, resolution time.Duration) (*opencost.AllocationSet, map[nodeKey]*nodePricing, error) {
	...
	queryRAMUsageAvg := fmt.Sprintf(queryFmtRAMUsageAvg, env.GetPromClusterFilter(), durStr, env.GetDefaultPromNamespaceLabel(), env.GetPromClusterLabel()) // DEFAULT NAMESPACE LABEL FOR VZ
	resChRAMUsageAvg := ctx.QueryAtTime(queryRAMUsageAvg, end)

	queryRAMUsageMax := fmt.Sprintf(queryFmtRAMUsageMax, env.GetPromClusterFilter(), durStr, env.GetDefaultPromNamespaceLabel(), env.GetPromClusterLabel()) // DEFAULT NAMESPACE LABEL FOR VZ
	...
	queryCPUUsageAvg := fmt.Sprintf(queryFmtCPUUsageAvg, env.GetPromClusterFilter(), durStr, env.GetDefaultPromNamespaceLabel(), env.GetPromClusterLabel()) // DEFAULT NAMESPACE LABEL FOR VZ
	resChCPUUsageAvg := ctx.QueryAtTime(queryCPUUsageAvg, end)

	queryCPUUsageMax := fmt.Sprintf(queryFmtCPUUsageMaxRecordingRule, env.GetPromClusterFilter(), durStr, env.GetDefaultPromNamespaceLabel(), env.GetPromClusterLabel()) // DEFAULT NAMESPACE LABEL FOR VZ
	...
	if len(resCPUUsageMax) == 0 {
		...
		queryCPUUsageMax = fmt.Sprintf(queryFmtCPUUsageMaxSubquery, env.GetPromClusterFilter(), doubleResStr, durStr, resStr, env.GetDefaultPromNamespaceLabel(), env.GetPromClusterLabel()) // DEFAULT NAMESPACE LABEL FOR VZ
		resChCPUUsageMax = ctx.QueryAtTime(queryCPUUsageMax, end)
		resCPUUsageMax, _ = resChCPUUsageMax.Await()
	...
	queryGPUsUsageAvg := fmt.Sprintf(queryFmtGPUsUsageAvg, durStr, env.GetPromNamespaceLabel(), env.GetPromClusterLabel())
	resChGPUsUsageAvg := ctx.Query(queryGPUsUsageAvg)

	queryGPUsUsageMax := fmt.Sprintf(queryFmtGPUsUsageMax, durStr, env.GetPromNamespaceLabel(), env.GetPromClusterLabel())
	...
	queryNodeCostPerRAMGiBHr := fmt.Sprintf(queryFmtNodeCostPerRAMGiBHr, env.GetPromClusterFilter(), durStr, env.GetPromNodeLabel(), env.GetPromClusterLabel())
	resChNodeCostPerRAMGiBHr := ctx.QueryAtTime(queryNodeCostPerRAMGiBHr, end)

	queryNodeCostPerGPUHr := fmt.Sprintf(queryFmtNodeCostPerGPUHr, env.GetPromClusterFilter(), durStr, env.GetPromNodeLabel(), env.GetPromClusterLabel())
